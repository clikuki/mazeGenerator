export class Cell {
    ...
    grayOut(canvas, ctx) {
        ...
        ctx.fillRect(Math.floor(this.screenX + (!this.screenX && this.walls[3] ? 0 : 1)), Math.floor(this.screenY + (!this.screenY && this.walls[0] ? 0 : 1)), Math.ceil(this.size + (this.screenX >= canvas.width && this.walls[1] ? 0 : 1)), Math.ceil(this.size + (this.screenY >= canvas.height && this.walls[2] ? 0 : 1)));
    }
    drawWalls(ctx) {
        // Walls
